...
'    Copyright (C) 2017-2026 Frank Steinberg and JBerg2026
...
'  05.05.2034	    Add SER3_TXDELAYus and SER3_TXDELAYms.  This is a post Ser1Send() to support serial LCDs etc 
  ...
  #script
    ...
    IF NODEF(SER3_INVERT) THEN
        SER3_INVERT = OFF
    END IF

    IF NODEF(SER3_TXPORT) THEN
        SER3_TXPORT = SER3_TXPORT_NOT_SET
    END IF

    IF NODEF(SER3_RXPORT) THEN
        SER3_RXPORT = SER3_RXPORT_NOT_SET
    END IF

    IF NODEF(SER3_DATABITS) THEN
         SER3_DATABITS = 8
    END IF

    IF NODEF(SER3_STOPBITS) THEN
         SER3_STOPBITS = 1
    END IF

    IF NODEF(SER3_RXNOWAIT) THEN
        SER3_RXNOWAIT  = OFF
    END IF
