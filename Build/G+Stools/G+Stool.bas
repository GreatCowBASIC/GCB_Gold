...
sTitle = " G+Stool      V20220127      www.FrankSteinberg.de      >>fst'22<<"
...
Sub runGCB(ByVal sBatFileName As String)
  ...
  ChDir sPathGCBSyn + "\GreatCowBasic"
...
Sub CheckGCBErrors()
  ...
  'If FileExists(sPathGCBSyn+"\GreatCowbasic\Errors.txt") Then
  If FileLen(sPathGCBSyn+"\GreatCowbasic\Errors.txt") Then
    ...
    Print ">>>  WARNINGs / ERRORs reported by Great Cow BASIC  (if Syntax Error, doubleclick on the errormessage below)  <<<"
     ...
     'If FileLen(sPathGCBSyn+"\GreatCowbasic\Errors.txt") > 3 Then
     ...
     Open sPathGCBSyn+"\GreatCowbasic\Errors.txt" For Input As #3
     Do While Not Eof(3)
      Print Input(1, #3);
     Loop
     Close #3
...
Sub DelProgammerLog()
  ...
  Kill sPathGCBSyn+"\GreatCowBasic\Programmer.log"
...
Sub ShowProgammerLog()
  ...
  If Not FileExists(sPathGCBSyn+"\GreatCowBasic\Programmer.log") Then Exit Sub

  '** parse logfile for info to display:
  Open sPathGCBSyn+"\GreatCowBasic\Programmer.log" For Input As #3
...
Sub CheckForIni()
  ...
  If FileExists(sPathGCBSyn+"\GreatCowBasic\gcbasic.ini") Then
    ErrorHandling("gcbasic.ini  is not compatible with GCB[at]Syn!" + Chr(10) + _
                  ...
                  sPathGCBSyn+"\GreatCowBasic\gcbasic.ini")
  End If
